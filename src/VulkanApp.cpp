...
void VulkanApp::createInstance() {
	...
	if (enableValidationLayers) {
		...
		instanceCreateInfo.pNext = (VkDebugUtilsMessengerCreateInfoEXT*)&createInfoExt;
	}
	else {
	...
	std::vector<VkExtensionProperties> extensionProps(extensionsCount);

	vkEnumerateInstanceExtensionProperties(nullptr, &extensionsCount, extensionProps.data());
}

bool VulkanApp::checkValidationLayersSupport() {
...
void VulkanApp::setupDebugMessenger() {
	...
	if (createDebugUtilsMessengerEXT(instance, &createInfoExt, nullptr, &debugUtilsMessengerExt) != VK_SUCCESS) {
		throw std::runtime_error("failed to setup debug messenger");
	}
}

VkResult
VulkanApp::createDebugUtilsMessengerEXT(VkInstance instance, const VkDebugUtilsMessengerCreateInfoEXT* pCreateInfo,
...
SwapChainSupportDetails VulkanApp::querySwapChainSupport(VkPhysicalDevice device) {
	...
	vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, surface, &details.capabilitiesKhr);
...
VkExtent2D VulkanApp::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& caps) {
...
void VulkanApp::createSwapChain() {
	...
	VkSurfaceFormatKHR sf = chooseSwapSurfaceFormat(swapChainSupport.surfaceFormats);
	VkPresentModeKHR pm = chooseSwapPresentMode(swapChainSupport.presentModes);
	VkExtent2D ex = chooseSwapExtent(swapChainSupport.capabilitiesKhr);

	uint32_t imagesCount = std::min(swapChainSupport.capabilitiesKhr.minImageCount + 1,
		swapChainSupport.capabilitiesKhr.maxImageCount);
	...
	createInfo.preTransform = swapChainSupport.capabilitiesKhr.currentTransform;
...
void VulkanApp::createGraphicsPipeline() {
	auto vertexShaderSPIRV = readShaderSPIRV("shaders/vert.spv");
	auto fragmentShaderSPIRV = readShaderSPIRV("shaders/frag.spv");

	VkShaderModule vertSM = createShaderModule(vertexShaderSPIRV);
	VkShaderModule fragSM = createShaderModule(fragmentShaderSPIRV);
...
std::vector<char> VulkanApp::readShaderSPIRV(const std::string& path) {
	...
	size_t fileSize = (size_t)fstream.tellg();
...
void VulkanApp::recordCommandBuffer(VkCommandBuffer buffer, uint32_t imageIndex) {
...
void VulkanApp::drawFrame() {
	...
	VkSemaphore waitSemapthores[] = { imageAvailableSemaphores[currentFrame] };
	VkPipelineStageFlags waitStages[] = { VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT };

	submitInfo.waitSemaphoreCount = 1;
	submitInfo.pWaitSemaphores = waitSemapthores;
...
void
VulkanApp::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer,
...
void VulkanApp::createDescriptorSetLayout() {
	...
	uboLayoutBind.pImmutableSamplers = nullptr; //optional

	VkDescriptorSetLayoutBinding samplerLayoutBing{};
	samplerLayoutBing.binding = 1;
	samplerLayoutBing.descriptorCount = 1;
	samplerLayoutBing.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	samplerLayoutBing.pImmutableSamplers = nullptr;
	samplerLayoutBing.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

	std::array<VkDescriptorSetLayoutBinding, 2> bindings = { uboLayoutBind, samplerLayoutBing };
...
void VulkanApp::updateUniformBuffer(uint32_t currentImage) {
	...
	UniformBufferObject ubo{ glm::mat4(1.0), glm::mat4(1.0), glm::mat4(1.0) };
...
void VulkanApp::createTextureImage() {
	...
	copyBufferToImage(staging, textureImage, width, height);
...
void VulkanApp::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, 
...
void VulkanApp::generateMipMaps(VkImage image, VkFormat format, int32_t width, int32_t height, uint32_t mipLevels)
{
...
VkSampleCountFlagBits VulkanApp::getMaxUsableSampleCount()
{
	...
	VkSampleCountFlags counts = physicalDeviceProps.limits.framebufferColorSampleCounts &
